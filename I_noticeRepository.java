...
public interface I_noticeRepository extends JpaRepository<I_noticeDto,Integer> {
	...
	@Query(value = " select * from I_notice where intctt like '%' || :search || '%' "
	...
	@Query(value = " select * from I_notice where indept = :indept "
				 + " order by case when intcca = '필수' then 0 else 1 end, intcno desc", 
		   countQuery = " select count(*) from I_notice where indept = :indept ", nativeQuery = true)
	// 분류에 따른 데이터 개수에 따라 페이지 재 정렬
	Page<I_noticeDto> findByIndeptContaining(@Param(":indept")String indept, Pageable pageable);
}